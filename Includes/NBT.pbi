...
Procedure NBT_Tag_Set_Array(*Tag.NBT_Tag, *Data_, Data_Size)
  ...
  Select *Tag\Type
    Case #NBT_Tag_Byte_Array
      If *Tag\Raw
        FreeMemory(*Tag\Raw) : *Tag\Raw = #Null
      EndIf
      If *Data_
  ...
  If Compression = #NBT_Compression_GZip Or Compression = #NBT_Compression_ZLib
    ...
    *End_Adress = NBT_Write_Ram_Helper(*NBT_Element\NBT_Tag, *Temp_Buffer)
    ...
    If Not deflateInit2_(Stream, #Z_DEFAULT_COMPRESSION, #Z_DEFLATED, Window_Size, 8, #Z_DEFAULT_STRATEGY, zlibVersion(), SizeOf(z_stream)) = #Z_OK
      NBT_Main\Error_String + "<"+#PB_Compiler_Procedure+"|"+Str(#PB_Compiler_Line)+": deflateInit2_(Stream, #Z_DEFAULT_COMPRESSION, #Z_DEFLATED, 15+16, 8, #Z_DEFAULT_STRATEGY, zlibVersion(), SizeOf(z_stream)) != #Z_OK>"
      FreeMemory(*Temp_Buffer)
      ProcedureReturn 0
    EndIf
...
Procedure NBT_Write_File(*NBT_Element.NBT_Element, Filename.s, GZip=#True)
...
EndIf
  ...
  If GZip
      Real_Size = NBT_Write_Ram(*NBT_Element, *Temp_Buffer, Temp_Buffer_Size, #NBT_Compression_GZip)
  Else
      Real_Size = NBT_Write_Ram(*NBT_Element, *Temp_Buffer, Temp_Buffer_Size, #NBT_Compression_None)
  EndIf
