  ...
  *Pointer.Network_Client = Client_Get_Pointer(Client_ID)
  If *Pointer
    If *Pointer\CPE = #True
      ...
      Elements = MapSize(*Pointer\Extensions())
      
      ResetMap(*Pointer\Extensions())
      
      While NextMapElement(*Pointer\Extensions())
        lua_pushstring(Lua_State, MapKey(*Pointer\Extensions()))
        lua_pushinteger(Lua_State, *Pointer\Extensions())
        lua_rawset(Lua_State, -3)
      Wend
...
ProcedureC Lua_CMD_PrintN(Lua_State)
  myString.s = ""
  lua_tostring(myString, Lua_State, 1)  
...
Procedure Lua_Register_All()
  If Lua_Main\State
    ...
    lua_register(Lua_Main\State, "printn", @Lua_CMD_PrintN())
  EndIf
EndProcedure
